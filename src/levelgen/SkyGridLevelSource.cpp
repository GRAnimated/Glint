...
SkyGridLevelSource::SkyGridLevelSource(Level* level, bool isGenerateMapFeatures, long long seed) {
    int xzSize = level->getLevelData()->getXZSize();
    float hellScale = level->getLevelData()->getHellScale();

    level->setSeaLevel(63);
    ...
    hk::gfx::imguiLog("SkyGridLevelSource::SkyGridLevelSource");
}

SkyGridLevelSource::~SkyGridLevelSource() {
    delete mSeed;
    delete mRandom;
}

void SkyGridLevelSource::prepareHeights(int x, int z, ChunkPrimer* primer) {
...
void SkyGridLevelSource::buildSurfaces(int x, int z, ChunkPrimer* primer) {
...
LevelChunk* SkyGridLevelSource::createChunk(int x, int z) {
    hk::gfx::imguiLog("Creating chunk at (%d, %d)\n", x, z);
...
void SkyGridLevelSource::postProcess(int x, int z) {
...
void SkyGridLevelSource::lightChunk(LevelChunk* chunk) {
    return chunk->recalcHeightmap();
}

bool SkyGridLevelSource::postProcessLoadedChunk(LevelChunk* chunk, int x, int z) {
    return false;
}

void SkyGridLevelSource::getMobsAt(MobCategory* category, BlockPos const& pos) {
    Biome* biome = mLevel->getBiome(pos);
    if (biome)
        biome->getMobs(category);
}

void* SkyGridLevelSource::findNearestMapFeature(Level* level, std::wstring const& name, BlockPos const& pos,
                                                bool flag) {
    return nullptr;
}

void SkyGridLevelSource::recreateLogicStructuresForChunk(LevelChunk* chunk, int x, int z) {
    return;
}

bool SkyGridLevelSource::isPosInFeature(Level* level, std::wstring const& name, BlockPos const& pos) {
    return false;
}
